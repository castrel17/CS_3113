...
constexpr int FRAMES_PER_SECOND = 4; //how fast the sprites are going to be alternating

//int g_animation_down[4] = {0,4,8,12}; //from the sprite sheet coordinates
//int g_animation_frame_set = SPRITESHEET_DIMENSIONS;
...
//this allows us to access one of the animation indecies at a time
...
//shader program, font texture id, what you want to write, font size, spacing, position (of the first letter)
void draw_text(ShaderProgram *shader_program, GLuint font_texture_id, std::string text,
               float font_size, float spacing, glm::vec3 position)
{
...
void process_input()
{
    ...
    //only animating if there is any movement
    
    //change animation frames when buttons are pressed
    //change movement to be in a direction as well as the animation frame
...
void update()
{
    ...
    {
        //1. keep track of how much time has passed
        g_animation_time += delta_time;
        float frames_per_second = (float) 1 / SECONDS_PER_FRAME;
        
        //2. if we have reached the beginning of a frame of a span
    ...
    /* TRANSFORMATIONS */ //remains the same
    g_george_matrix = glm::mat4(1.0f);
...
void render()
{
    ...
    //use this instead of the draw object function
    //g_animation_indices[g_animation_index], want to render the index within the animation indeces
    ...
    draw_text(&g_shader_program, g_font_texture_id, "ELIZABETH!", 0.5f, 0.05f,
